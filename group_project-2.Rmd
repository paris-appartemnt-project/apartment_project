---
title: "group_project"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load libraries 
```{r}
# Load libraries
library(dplyr)
library(rvest)
library(purrr)
library(readr)
library(tidyr)
library(data.table)
library(stringr)
library(base)
library(stringi)

# Clear workspace
rm(list=ls())
```

## Data compilation

### Schools database
```{r}
## Schools data set : keeping only 
#schools <- schools %>% 
#  filter(localite_acheminement_uai == "PARIS")
#write.csv(schools, "schools_paris.csv")
```

### Castorus database
```{r}
## Step 1 : Create a dataframe with all the ads on Castorus website 
# Generalize it for different url 
get_table <- function(url) {
  table <- url %>%
  html_nodes("table") %>% 
  html_table(header = NA)
  table <- as.data.frame(table)
  return(table)
}

# Create of a list of all the url (to gather all the different ads)
base_url <- "https://www.castorus.com/s/Paris,51185,2-2---"
list_url <- paste0(base_url,seq(0, 2990000, by = 10000),"-", seq(9900, 3000000, by = 10000))
list_url2 <- list_url %>% 
  map(read_html)

# Create a dataframe with all the ads on the website 
castorus_table <- map(list_url2, get_table) 
castorus_table <- rbindlist(castorus_table)
castorus_table <- as.data.frame(castorus_table)
# Extract the street from the "ville" variable 
castorus_table <- castorus_table %>% separate(ville,into = c("ville","rue"),sep = "[(:)]",remove = TRUE) 
```



```{r}
## Step 2 : Add to the data previously created all the descriptions (that are on a different html page)
# Generalize it for different url 
get_link <- function(url){
  link <- url %>% 
  html_nodes("table") %>% 
  html_nodes("a") %>% 
  html_attr("href")
  link <- as.data.frame(link)
  return(link)
}

# Create a list with all url with the descriptions of the ads 
url_description <- map(list_url2, get_link)
url_description_df <- rbindlist(url_description)
colnames(url_description_df) <- "links"
url_description_ls <- url_description_df %>% 
  pull("links")
base_url_description <- "https://www.castorus.com"
list_url_description <- paste0(base_url_description,url_description_ls)
castorus_table <- cbind(castorus_table,as.data.frame(list_url_description))  
castorus_table$list_url_description <- as.character(castorus_table$list_url_description)

## Data cleanup
castorus_table$prix <- as.character(castorus_table$prix)
#Remove euro sign from prix column
castorus_table$prix <- gsub("[^[:alnum:][:blank:]?&/\\-]", "", castorus_table$prix)
#Remove spaces in between characters
castorus_table$prix <- gsub("[[:space:]]", "", castorus_table$prix) 
#Remove zeroes in front of first non zero digit
castorus_table$prix <- gsub("^[0]+", "", castorus_table$prix) 
#Convert prix to numeric
castorus_table$prix <- as.numeric(castorus_table$prix)

# While looking at the data it is evident that there are some anormalities within the data. This is due to the fact that some of the flats are for rent and not for sale. 
# It is necessary to filter out the studios which will have a small price and the flats which will have about the same price but which are on rent.
castorus_table <-  as.data.frame(castorus_table)

# Select flats when they have more than 1 room and a price higher than 50000€ or when there is only 1 room so a studio. 
castorus_table <- castorus_table %>% 
  mutate(Type = case_when((piec. != 1 & prix >= 50000) | (piec. == 1) ~ 1, 
                                              TRUE~ 0 
                                              )
                                            )

castorus_table <- castorus_table %>% filter(Type == 1)

write_csv(castorus_table,"castorus_table1.csv") 

castorus_table$ville <- tolower(castorus_table$ville)
castorus_table <- castorus_table %>% dplyr::filter(grepl("paris", ville))
castorus_table <- castorus_table %>% separate(ville,into = c("ville","arrondissement"),sep = "(?<=[a-zA-Z])\\s*(?=[0-9])",remove = TRUE) 
castorus_table$arrondissement <- trimws(castorus_table$arrondissement)

# Create a function to deal with errors in reading html
try_catch_url <- function(url){
  tryCatch(
    url %>%
      read_html(),
    error = function(e){NA}    # a function that returns NA regardless of what happened
  )
}

list_description_final<- castorus_table$list_url_description %>% map(try_catch_url)

# Extract the description from the ads - Generalisation for all the ads


get_description <- function(url) {
  description <- tryCatch(
     url %>%
  html_nodes(".contenu_bw") %>% 
  html_text %>% 
  unlist(), error = function(e){NA})
  description <- as.data.frame(description[8])
  return(description)
}

# Create a dataframe with the descriptions of all the ads
castorus_description <- map(list_description_final, get_description)
castorus_description <- rbindlist(castorus_description) 
castorus_description <- as.data.frame(castorus_description) %>% rename(Description = 'description[8]') 
castorus_description <- castorus_description %>%  as.data.frame()

Unaccent <- function(text) {
  text <- gsub("['`^~\"]", " ", text)
  text <- iconv(text, to="ASCII//TRANSLIT//IGNORE")
  text <- gsub("['`^~\"]", "", text)
  return(text)
}

castorus_description$Description <- tolower(castorus_description$Description)
castorus_description$Description <- Unaccent(castorus_description$Description)
castorus_description$Description <- gsub("a\\(c\\)","e",castorus_description$Description)
castorus_description$Description <- gsub("[^[:alnum:][:blank:]?&/\\-]", "", castorus_description$Description)

castorus_data <- cbind(castorus_table, castorus_description) 
castorus_data <- castorus_data %>% dplyr::filter(str_detect(Description, 'extrait de la derniere description'))
castorus_data$Description <- gsub(".*:","",castorus_data$Description) 

# Combine the two tables (ads and description)
write_csv(castorus_description, "castorus_description.csv" )
write_csv(castorus_data, "castorus_data.csv")
a1<-load("/Users/po/Downloads/castorus_description.RData")
castorus_data <- as.data.frame(castorus_data)
View(castorus_data)
```

